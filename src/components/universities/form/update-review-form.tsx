...
export default function UpdateReview({ review, universityId }: { review: Review; universityId: string }) {
  const { data: session } = useSession()
  const accessPath = usePathname()

  const createReviewWithUniversityId = updateReview.bind(null, review.id, review.universityId, accessPath)
  ...
  const [state, formAction] = useFormState(createReviewWithUniversityId, initialState)
      ...
      <div className='flex justify-center p-4 md:m-auto md:w-7/12'>
        <div className='flex w-full flex-col gap-4 rounded-md bg-gray-100 p-4'>
          ...
          <div className='flex flex-col gap-2'>
            ...
            <select id='faculty' name='faculty' className='p-2' defaultValue={review.faculty}>
          ...
          <div>
            ...
            <div className='flex '>
              {stars.map((value, element) => (
                <div key={element} onClick={() => onClickHandler(value)} className='hover: cursor-pointer'>
                  {element >= rating ? (
          ...
          <div>
            ...
            <div className='mt-2 flex flex-col gap-4 rounded-md border border-gray-200 bg-white p-4'>
              <div>
                <input id='anonymous' type='radio' name='who' value='anonymous' defaultChecked />

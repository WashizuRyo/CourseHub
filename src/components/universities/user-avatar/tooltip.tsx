        ...
        {session?.user ? (
          <div className='rounded bg-blue-200 text-gray-700'>
            <div className='flex items-center'>
              <Image src={session.user.image!} alt='user image' width={38} height={38} className='m-2 rounded-full' />
              <Link href={`/users/${session.user.id}/reviews`} className='break-all hover:text-gray-500' id='username'>
                {session.user?.name}
              </Link>
            </div>
